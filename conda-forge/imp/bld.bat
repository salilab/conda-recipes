...
if "%VisualStudioVersion%" == "" (
  mkdir include
  copy %RECIPE_DIR%\msvc2008-stdint\stdint.h %PREFIX%\Library\include\stdint.h
  if errorlevel 1 exit 1
)
...
set NUMPY_INC="-DPython2_NumPy_INCLUDE_DIR=%PREFIX%\lib\site-packages\numpy\core\include"
if "%PY3K%" == "1" (
  set USE_PYTHON2=off
  set NUMPY_INC="-DPython3_NumPy_INCLUDE_DIR=%PREFIX%\lib\site-packages\numpy\core\include"
)

cmake -DUSE_PYTHON2=%USE_PYTHON2% %OLDPYTHON% %NUMPY_INC% ^
  -DCMAKE_PREFIX_PATH="%BUILD_PREFIX:\=/%;%BUILD_PREFIX:\=/%\Library" ^
  -DCMAKE_BUILD_TYPE=Release -DIMP_DISABLED_MODULES=scratch:cnmultifit ^
  ...
  -Dopencv_core_LIBRARY="%BUILD_PREFIX:\=/%/Library/lib/opencv_core%OPENCV_VER%.lib" ^
  -Dopencv_imgproc_LIBRARY="%BUILD_PREFIX:\=/%/Library/lib/opencv_imgproc%OPENCV_VER%.lib" ^
  -Dopencv_highgui_LIBRARY="%BUILD_PREFIX:\=/%/Library/lib/opencv_highgui%OPENCV_VER%.lib" ^
  -Dopencv_imgcodecs_LIBRARY="%BUILD_PREFIX:\=/%/Library/lib/opencv_imgcodecs%OPENCV_VER%.lib" ^
  -DHDF5_LIBRARIES="%BUILD_PREFIX:\=/%/Library/lib/hdf5.lib" ^
  -DHDF5_FOUND=TRUE -DHDF5_INCLUDE_DIRS="%BUILD_PREFIX:\=/%/Library/include" ^
  -DHDF5_INCLUDE_DIR="%BUILD_PREFIX:\=/%/Library/include" ^
  -Dprotobuf_LIBRARY="%BUILD_PREFIX:\=/%/Library/lib/libprotobuf.lib" ^
  %PERCPPCOMP% -G "Ninja" ..
if errorlevel 1 exit 1
